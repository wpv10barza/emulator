...
async function bridge(request, response, url) {
  ...
  // Unico camino de escritura humana: confirma o rechaza un comando pendiente.
  const decisionMatch = url.pathname.match(/^\/bridge\/commands\/([A-Za-z0-9-]+)\/(confirm|reject)$/);
  if (request.method === "POST" && decisionMatch) {
    const input = await readBody(request);
    const result = await callBackend("POST", `/api/device/v1/commands/${decisionMatch[1]}/${decisionMatch[2]}`, input);
    return sendJson(response, result.status, result.payload);
  }
